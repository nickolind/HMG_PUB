...
_va = 0.1 + random 0.9;
// _va = random [0.1,0.8,1];
_vb = 0;	// Дождь пока что всегда выключен, т.к. глючит и если включается, то на полную катушку, игнорируя заданное значение
// _vb = 0 max (random (((0.8 min _va) - 0.6) max 0) - 0.05); // if overcast > 0.6 --> rain chance 38% ; maxRain < 0.15
_vc = 0 max (random ((_va - 0.8) max 0) - 0.15);	// if overcast > 0.8 --> lightning chance 5%
...
["hp_roundPause", "create", _roundPauseTime] call NSA_Timer;
...
["hp_roundPause", "delete"] call NSA_Timer;
